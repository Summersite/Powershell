...
Copy-Item  -PassThru $SourcePathData $DestPath\$($hostname)\dropbox -Force -Recurse >> $logfile
Write-Host -ForegroundColor Green "I've just copied $SourcePathData to $SourcePathDatap to $DestPath\$($hostname)\data" >> $logfile
Copy-Item  -PassThru $SourcePathBookMarks $DestPath\$($hostname)\bookmarks -Force -Recurse >> $logfile
